/*
	...
	# 제약 조건
		...
		- NOT NULL : NULL 금지
		- UNIQUE : 중복값입력 금지(NULL은 가능)
		- PRIMARY KEY(PK) : # PRIMARY : 주요한. KEY는 연결고리
...
CREATE TABLE T_SHOPPING_GOODS(
    GOODS_ID 				INT 		AUTO_INCREMENT PRIMARY KEY,		# AUTO_INCREMENT은 숫자 1씩 자동 증가
    GOODS_TITLE 			VARCHAR(50) UNIQUE NOT NULL,
    GOODS_WRITER 			VARCHAR(50),
	GOODS_PRICE 			INT 		NOT NULL
);

INSERT INTO T_SHOPPING_GOODS VALUES(1 , "쉽게 배우는 데이터 통신" , "박기현" , 19000);
INSERT INTO T_SHOPPING_GOODS VALUES("컴퓨터 활용 능력" , "이주희" , 38000);	# AUTO_INCREMENT 썼어도 개수 불일치하면 안됨
...
INSERT INTO 
			T_SHOPPING_GOODS(GOODS_TITLE, GOODS_WRITER, GOODS_PRICE) 
VALUES		
			("컴퓨터 활용 능력" , "이주희" , 25000);            				# UNIQUE 제약 위반해서 데이터 등록 안됨
...
VALUES		
			("아무튼,외국어2" , "조지영");									# NOT NULL 제약 위반해서 데이터 등록 안됨
...
CREATE TABLE T_STUDENT2(
    ...
    STUDENT_NAME VARCHAR(10),
    CLASS_ID 	 VARCHAR(10),
    FOREIGN KEY (CLASS_ID) REFERENCES T_CLASS2(CLASS_ID)	# 내 FOREIGN KEY로 T_CLASS2(CLASS_ID)를 참조한다.
);
...
SET
		CLASS_ID = "C100"
WHERE
		CLASS_ID = "C1";	# 참조키가 있을 경우 메인키의 값만 수정 불가

DELETE FROM
		T_CLASS2
WHERE
		CLASS_ID = "C2";    # 참조키가 있을 경우 메인키의 값만 삭제 불가
...
CREATE TABLE T_STUDENT3(
    ...
    STUDENT_NAME VARCHAR(10),
    CLASS_ID 	 VARCHAR(10),
    PRIMARY KEY (STUDENT_ID),								# 이렇게 따로 써도 됨.
    FOREIGN KEY (CLASS_ID) REFERENCES T_CLASS3(CLASS_ID)	# 내 FOREIGN KEY로 T_CLASS2(CLASS_ID)를 참조한다.
