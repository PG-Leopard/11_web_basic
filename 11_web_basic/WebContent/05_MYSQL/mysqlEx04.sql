...
SELECT
        ...
        SALARY * 0.033,
        SALARY - (SALARY * 0.033),
        SALARY * 0.033 			  AS TAX,		# AS 키워드를 이용하여 컬럼명을 지정할 수 있다. MYSQL의 주석은 #
        SALARY - (SALARY * 0.033) AFTER_TAX		# as 키워드는 생략 가능하다. 실무에선 붙여서 사용하는게 국룰
FROM
		SALARIES;								# 테이블에도 as 키워드 가능한데, 테이블에는 as를 생략해서 사용하는게 국룰
...
FROM
 ...
 /*
    ...
    Ex)
		LIMIT 3      : 3row만 조회
		LIMIT 12     : 12row만 조회
		LIMIT 10,3   : 10번 인덱스부터 3row만 조회
		LIMIT 100,10 : 100번 인덱스부터 10row만 조회
