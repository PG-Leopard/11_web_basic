...
FROM
        ...
        INNER JOIN T_ORDER O
				ON P.PRODUCT_CODE = O.PRODUCT_CODE
			   AND ORDER_DATE >= '2020-01-01' AND ORDER_DATE <='2020-12-31';
               
# 문제 3) '상품별'로 2020년 동안 판매된 수량(내림차순)순으로 정렬하여  상품코드 , 상품이름 , 상품판매량 조회하기.
SELECT
        P.PRODUCT_CODE,
        P.PRODUCT_NAME,
        SUM(O.ORDER_GOODS_QTY) AS ANNUAL_2020_TOTAL_SALES
FROM
		T_PRODUCT P
        INNER JOIN T_ORDER O
				ON P.PRODUCT_CODE = O.PRODUCT_CODE
...
GROUP BY
		P.PRODUCT_CODE
ORDER BY 
		ANNUAL_2020_TOTAL_SALES DESC;

# 문제 4) '지역별'로 판매량이 많은 순서대로 정렬하여 지역명과 판매량 조회하기.
SELECT
		M.REGIDENCE,
        SUM(O.ORDER_GOODS_QTY) AS EACH_RESIDENCE_SALES
FROM
		  T_MEMBER M
        INNER JOIN T_ORDER O
...
SELECT
		P.PRODUCT_CODE,
        P.PRODUCT_NAME,
        SUM(P.PRODUCT_PRICE * O.ORDER_GOODS_QTY + P.PRODUCT_DELIVERY_PRICE) AS PRODUCT_SALES_SUM
FROM
		 T_PRODUCT P
        INNER JOIN T_ORDER O
				ON P.PRODUCT_CODE = O.PRODUCT_CODE
...
SELECT
		SUM(P.PRODUCT_PRICE * O.ORDER_GOODS_QTY + P.PRODUCT_DELIVERY_PRICE) AS MERKEL
FROM 
		T_PRODUCT P
	  INNER JOIN T_ORDER O
			  ON P.PRODUCT_CODE = O.PRODUCT_CODE
